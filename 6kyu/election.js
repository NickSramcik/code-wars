...
// Store a candidate map
// Loop through input array
//      Set or increment candidate in map
// Store winning candidate
// Loop through candidate map
//      If current candidate has more votes than max, reassign max variables
// If winning candidate's votes > length / 2, return that candidate
// Otherwise return null

function getWinner(ballots) {
    const candidates = {};
    let winning = ballots[0];

    for (vote of ballots) {
        candidates[vote] = candidates[vote] + 1 || 1;
    }

    for (current in candidates) {
        if (candidates[current] > candidates[winning]) {
            winning = current;
        }
    }

    return candidates[winning] > ballots.length / 2 ? winning : null;
}
...
function getWinner(ballots) {
    ...
    return candidates[winning] > plurality ? winning : null;
}