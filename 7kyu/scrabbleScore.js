// Write a program that, given a word, computes the scrabble score for that word.

// Letter Values
// You'll need these:

// Letter                           Value
// A, E, I, O, U, L, N, R, S, T       1
// D, G                               2
// B, C, M, P                         3
// F, H, V, W, Y                      4
// K                                  5
// J, X                               8
// Q, Z                               10
// There will be a preloaded hashtable $dict with all these values: $dict["E"] == 1.

// Examples
// "cabbage" --> 14
// "cabbage" should be scored as worth 14 points:

// 3 points for C
// 1 point for A, twice
// 3 points for B, twice
// 2 points for G
// 1 point for E
// And to total:

// 3 + 2*1 + 2*3 + 2 + 1 = 3 + 2 + 6 + 3 = 14

// Empty string should return 0. The string can contain spaces and letters (upper and lower case), you should calculate the scrabble score only of the letters in that string.

// Pass in a string of letters
// Return a number -> the score of the input string
// ""           --> 0
// "STREET"    --> 6
// "st re et"    --> 6
// "ca bba g  e" --> 14

// Store scores in a hash map
// Store score value, set it to 0
// Loop through the input string,
//      check each letter in the hash map & add score to count
// Return the score

function scrabbleScore(string) {
    const scrabbleScores = {
        'a': 1,
        'e': 1,
        'i': 1,
        'o': 1,
        'u': 1,
        'l': 1,
        'n': 1,
        'r': 1,
        's': 1,
        't': 1,
        'd': 2,
        'g': 2,
        'b': 3,
        'c': 3,
        'm': 3,
        'p': 3,
        'f': 4,
        'h': 4,
        'v': 4,
        'w': 4,
        'y': 4,
        'k': 5,
        'j': 8,
        'x': 8,
        'q': 10,
        'z': 10
    };

    let score = 0;

    for (letter of string) {
        if (scrabbleScores[letter.toLowerCase()]) {
            score += scrabbleScores[letter.toLowerCase()];
        }
    }

    return score;
}

console.log(scrabbleScore(''), '= 0')
console.log(scrabbleScore('STREET'), '= 6')
console.log(scrabbleScore('st re et'), '= 6')
console.log(scrabbleScore('ca bba g  e'), '= 14')

// "STREET"    --> 6
// "st re et"    --> 6
// "ca bba g  e" --> 14
