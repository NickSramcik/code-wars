...
// let max be the maximum value in input array
// Reduce the array, accumulate the difference b/w max and current value
// Return result

function candies(kids) {
    if (kids.length < 2) return -1;
    const max = Math.max(...kids);
    return kids.reduce((acc, cur) => acc + (max - cur), 0);
}
