...
// Pass in a string
// Return the string trimmed, and repeated once more each line for a total of 5 lines, with \n as line separator
// '  shrek  ' -> 'shrek/nshrekshrek/nshrekshrekshrek/nshrekshrekshrekshresk/nshrekshrekshrekshrekshrek
// ^ Pretend /n is \n

// Trim the input, then run a loop with the index representing how often the input repeats
...
// Using array.from, mapping the array to repeat per index, and joining into string
// First value needs to be trimmed off, because index 0 makes an empty value

const fiveLine = s => Array.from({length: 6}, (_, i) => s.trim().repeat(i)).slice(1).join('\n');