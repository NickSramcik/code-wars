...
// Pass in a string and an integer
// return input string with n stars on either side, alternating
// 'shrek', 4 -> '**shrek**'
// Use a padstart and padend. Round up the start, to give more stars on the left for odd n values

function padIt(str,n){
    return str.padStart(str.length + Math.ceil(n / 2), '*').padEnd(str.length + Math.floor(n), '*');
  }

// One line

const padIt = (s, n) => s.padStart(s.length + Math.ceil(n / 2), '*').padEnd(s.length + Math.floor(n), '*');
...
// sumAtoB(1, 10) -> 1 + 2 + 3... 9 + 10 = whatever number


// const shortcut = (a, b) => (b - a + 1) * (a + b) / 2;
