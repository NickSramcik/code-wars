...
// Return that array with each element cut down to the same length as the shortest element
// ['ogres', 'are', 'like', 'onions'] -> ['ogr', 'are', 'lik', 'oni']

// Map the input array to an array of element lengths, and store the minimum value into a variable
// Map the input array again, this time slicing each element to match minimum length. Return this
...
// Using a reduce to find minimum length

function cutIt(arr){
    const minLength = arr.reduce((a, c) => c.length < a ? c.length : a, Infinity);
    return arr.map(e => e.slice(0, minLength));
  }